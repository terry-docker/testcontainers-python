...
class ComposeContainer:
    ...
    def get_container_host_ip(self) -> str:
        ...
        # Use the same logic as get_service_host for consistency
        try:
            # Try to get the URL from any publisher for this container
            if self.Publishers:
                return self.Publishers[0].normalize().URL or "127.0.0.1"  # type: ignore[attr-defined]
            return "127.0.0.1"
    ...
    def get_exposed_port(self, port: int) -> int:
        ...
        try:
            publisher = self.get_publisher(by_port=port)
            published_port = publisher.normalize().PublishedPort  # type: ignore[attr-defined]
            return int(published_port) if published_port else port
        except (NoSuchPortExposed, ValueError, TypeError):
            # If no specific port mapping found or conversion fails, return the original port
            return port
